...
public class LGRoleDistributor {
    ...
    private void instantiateRole(Entry<String, Constructor<? extends Role>> currentRole) {
        try {
    ...
    private Role instantiateRole() {
        try {
            final Role createdRole = this.rolesBuilder.get("ChasseurDeVampire").newInstance(this.game);
            this.roles.add(createdRole);
            ...
            throw new RuntimeException("Failed to instantiate role: " + "ChasseurDeVampire", e);
        }
    }
    ...
    private List<Role> useRandomAssignation() {
        ...
        final Map<String, Object> evilWeigths = Objects.requireNonNull(this.config.getConfigurationSection(MainLg.DISTRIBUTION_RANDOM_KEY + "evilWeigths")).getValues(false);
        ...
        final Map<String, Object> neutralWeights = Objects.requireNonNull(this.config.getConfigurationSection(MainLg.DISTRIBUTION_RANDOM_KEY + "neutralWeights")).getValues(false);
        ...
        final Map<String, Object> villagerWeights = Objects.requireNonNull(this.config.getConfigurationSection(MainLg.DISTRIBUTION_RANDOM_KEY + "villagerWeights")).getValues(false);

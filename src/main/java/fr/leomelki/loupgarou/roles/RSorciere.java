...
public class RSorciere extends Role {
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (item.getItemMeta().getDisplayName().equals(Objects.requireNonNull(items[0].getItemMeta()).getDisplayName()) && sauver != null) {// Potion
            // de
            // vie
            e.setCancelled(true);
            closeInventory(player);
            saveLife(lgp);
        } else if (item.getItemMeta().getDisplayName().equals(Objects.requireNonNull(items[1].getItemMeta()).getDisplayName())) {// Cancel
            e.setCancelled(true);
            closeInventory(player);
            lgp.sendMessage(Role.PERFORMED_NO_ACTION);
            lgp.hideView();
            callback.run();
        } else if (item.getItemMeta().getDisplayName().equals(Objects.requireNonNull(items[2].getItemMeta()).getDisplayName())) {// Potion de mort
            e.setCancelled(true);

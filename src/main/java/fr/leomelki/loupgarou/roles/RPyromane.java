...
public class RPyromane extends Role {
    ...
    static {
        ...
        if (meta != null) {
            ...
            meta.setLore(Collections.singletonList("§8Recouvres deux joueurs d'essence"));
        }
    ...
    protected void onNightTurnTimeout(LGPlayer player) {
        if (first != null) {
            List<LGPlayer> liste = player.getCache().get(RPyromane.COVERED_IN_GASOLINE);
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (lgp.getRole() != this || item == null || item.getItemMeta() == null)
            return;
        if (item.getItemMeta().getDisplayName().equals(Objects.requireNonNull(nothing.getItemMeta()).getDisplayName())) {
            lgp.stopChoosing();
            closeInventory(player);
            lgp.hideView();
            lgp.sendMessage(Role.PERFORMED_NO_ACTION);
            callback.run();
        } else if (item.getItemMeta().getDisplayName().equals(Objects.requireNonNull(items[3].getItemMeta()).getDisplayName())) {
            e.setCancelled(true);
            closeInventory(player);
            if (!lgp.getCache().<List<LGPlayer>>get(RPyromane.COVERED_IN_GASOLINE).isEmpty()) {
                List<LGPlayer> liste = lgp.getCache().get(RPyromane.COVERED_IN_GASOLINE);
    ...
    public void onClick(PlayerInteractEvent e) {
        ...
        if (lgp.getRole() == this && e.getItem() != null && e.getItem().hasItemMeta()
                && Objects.requireNonNull(e.getItem().getItemMeta()).getDisplayName().equals(Objects.requireNonNull(cancel.getItemMeta()).getDisplayName())) {
            e.setCancelled(true);
            player.getInventory().setItem(8, null);
            player.updateInventory();
            lgp.stopChoosing();
